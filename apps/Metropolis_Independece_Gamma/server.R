...
shinyServer(function(input, output) {
   ...
   output$plot1 <- renderPlot({
     ...
     dados<- data.frame(iter=1:length(theta[,1]), 
                        theta1=theta[,1],
                        theta2=theta[,2], 
                        theta3=theta[,3])
     traceplot<- ggplot() + 
       geom_line(aes(x=iter,y=theta1), dados, color="black", size=2)+
       geom_line(aes(x=iter,y=theta2), dados, color="red",size=2)+
       geom_line(aes(x=iter,y=theta3), dados,color="blue",size=2)+
       xlab("Iterations")+
