...
fluidPage(title="Gibbs Sampling",
    ...
    sidebarPanel(
      ...
      h3(sliderInput("Iterations80",
                  "Iterations",
                  value = 100,min=100,max=100000, 
                  step=1000,animate=TRUE)),
      h3(sliderInput(inputId="burnin80", 
                   label="Burnin", 
                   value=1, min = 1, max = 5000,
                   step=100,animate=TRUE)),
      h3(sliderInput(inputId="thin80", 
                   label="Thin", 
                   value=1, min = 1, max = 1000,
                   step=10,animate=TRUE)),
      h3(strong("Initial values",style="color:black")),
      h3(numericInput(inputId="theta.inicial.1.80", 
                   label=HTML("&theta;[1]"), 
                   value=0.1, min = 0, max = 1,
                   step=0.1)),
      h3(numericInput(inputId="theta.inicial.2.80",
                   label=HTML("&theta;[2]"), 
                   value=0.5, min = 0, max = 1,
                   step=0.1)),
      h3(numericInput(inputId="theta.inicial.3.80",
                   label=HTML("&theta;[3]"), 
                   value=0.9, min = 0, max = 1,
                   step=0.1)),
      br(),
      br(),
      h3(strong("Parameter of Beta Binomial distribution",style="color:black")),
      h3(numericInput(inputId="n80",
                   label="n", 
                   value=16, min = 1, max = 100,
                   step=1)),
      h3(numericInput(inputId="alpha80", 
                   label=HTML("&alpha;"), 
                   value=2, min = 0.5, max = 10,
                   step=0.5)),
      h3(numericInput(inputId="beta80", 
                   label=HTML("&beta;"), 
